...
const TYPE_LABEL: Record<string, string> = {
    daily: twh('dailyType'),
    hourly: twh('hourlyType'),
    weekly: twh('weeklyType'),
    interval: twh('intervalType'),
}
            ...
            <div className="flex items-center gap-2">
                {maxHours > 0 && (
                    <>
                        <div className="flex-1 relative">
                            ...
                            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-zinc-600">{twhc('hourShrt')}</span>
                        </div>
                ...
                <div className="flex-1 relative">
                    <input type="number" min={maxHours > 0 ? 0 : 1} max={59} value={value.mins} disabled={disabled}
                        ...
                        className="w-full disabled:opacity-50 disabled:bg-zinc-900 bg-black border border-zinc-700 text-white text-xs rounded-lg px-3 py-2.5 outline-none focus:border-zinc-500 transition-colors text-center" />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-zinc-600">{twhc('minuteShrt')}</span>
                </div>
            </div>
        </div>
    )
...
const TimerProfileEdit = () => {
    ...
    const handleBeforeSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        setError(null)
        if (!name.trim()) { e.preventDefault(); setError(t('giveName')); return }
        if (!sites.length) { e.preventDefault(); setError(t('noSites')); return }

        const totalMins = duration.hours * 60 + duration.mins
        if (profileType !== 'interval' && totalMins === 0) { e.preventDefault(); setError(t('noLimit')); return }
        if (profileType === 'interval' && !rules.length) { e.preventDefault(); setError(t('noRules')); return }
        if (profileType === 'interval' && rules.some(r => !r.days.length)) {
            e.preventDefault(); setError(t('eachRuleMustHave')); return
        }
        ...
        <Form method="POST" onSubmit={handleBeforeSubmit}>
            ...
            <div className="max-w-2xl mx-auto space-y-6 pb-8">
                        ...
                        <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction
                                    ...
                                    <Trash2 width="14" />{tc('delete')}</AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
                    </AlertDialog>
                </div>
                ...
                <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-5 space-y-3">
                    <p className="text-[10px] font-medium text-zinc-400 uppercase tracking-wider">{t('profileName')}</p>
                ...
                <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-5 space-y-5">
                    <p className="text-[10px] font-medium text-zinc-400 uppercase tracking-wider">{t('configuration')}</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">
                                    {tc('days')} <span className="normal-case text-zinc-600">({t('emptyDays')})</span>
                                </p>
                    ...
                    {profileType === 'hourly' && (
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">
                                    {t('timeRanges')} <span className="normal-case text-zinc-600">({t('emptyRanges')})</span>
                                </p>
                                {!isStrict && (
                        ...
                        <div className="space-y-4">
                            {!isStrict && (
                                <>
                                    ...
                                    <div className="space-y-2">
                                        <p className="text-[10px] text-zinc-400">{tc('days')}</p>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {rules.map((rule, i) => (
                                        <div key={i} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 space-y-3">
                                            <div className="flex items-center justify-between">
                                                <span className="text-xs text-zinc-400 font-medium">{t('rule')} {i + 1}</span>
                                                {!isStrict && (
                                            ...
                                            <div className="space-y-1.5">
                                                <p className="text-[10px] text-zinc-600">{t('activeDays')}</p>
                                                <div className="flex gap-1.5 flex-wrap">
                ...
                <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-5 space-y-5">
                                    ...
                                    {sites.map((site, i) => (
                                        <tr key={i} className="hover:bg-zinc-900/30">
                                            <td className="px-4 py-3 flex items-center gap-3">
                                                <SmartImage 
                                                    src={`https://www.google.com/s2/favicons?domain=${site}&sz=28`}
                    ...
                    {!isStrict && (
                        <div className="space-y-2">
                            <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{t('quickSuggestions')}</p>
                    ...
                    {!isStrict && (() => {
                        ...
                        return (
                            <div className="space-y-2">
                                <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{t('topSites')}</p>
                                        ...
                                        return (
                                            <button key={domain} type="button"
                                                ...
                                                {sites.includes(domain)
                                                    ? <CheckCheckIcon fill='currentColor' stroke='#000' width="11" className="text-white shrink-0" />
                    ...
                    {!isStrict && (
                        <div className="space-y-2">
                            <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{t('addManually')}</p>
